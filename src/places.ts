 ...
 * append Korean travel hashtags, and publish it as a text post.
 *
 * Korean only — no EN/TH translation or replies, no image. Needs only
 * THREADS_ACCESS_TOKEN. Pass --dry-run to print the post without publishing.
 */
async function main(): Promise<void> {
  ...
  const id = await publishPost(token, ko);
